...
status_prompt = "Give me 3 random words"
update_minutes = 1
...
def generate_bio(prompt, model="curie", max_tokens=70):
    try:
...
def limit_string_length(string, length=32):
...
with vrchatapi.ApiClient(vrc_creds) as api_client:
    ...
    def update_bio(bio, status):
        # Generate a request
        usersapi = vrchatapi.UsersApi(api_client)
        update_user_request = vrchatapi.UpdateUserRequest(bio=bio)  # set bio
        update_user_request.status_description = status  # set status
        ...
        try:
            ...
            print(f"Exception when calling UsersApi->update_user: {e}")
    ...
    while True:
        ...
        new_bio = "Bio and status generated by ChatGPT every " + str(update_minutes) + " minutes.\n" + new_bio
        logging.info("Censored bio: " + new_bio)
        print("Newly generated bio:\n", new_bio)

        # Generate a status
        new_status = generate_bio(status_prompt, max_tokens=20)
        new_status = new_status.replace("\n", " ").replace("\r", "")
        logging.info("Raw status: " + new_status)
        new_status = censor_string(new_status, censor_list)
        new_status = limit_string_length(new_status)
        logging.info("Censored status: " + new_status)
        print("Newly generated status:\n", new_status)

        # Update bio and status
        update_bio(str(new_bio), str(new_status))
